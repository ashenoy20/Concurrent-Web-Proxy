...
//Public variables so threads have access to them
...
pthread_mutex_t mutex_lock = PTHREAD_MUTEX_INITIALIZER; //mutex to avoid file corruption
...
int main(int argc, char **argv)
{
	...
	if(pid == 0){
		...
		Pthread_create(&tid, NULL, &handleRequestThread, NULL);
		Pthread_detach(tid);
	}
...
void* handleRequestThread(void* args){
    ...
    int connectionfd = connfd; //connfd will change so we save the actual val 				      in a local variable
     ...
     */
    if (error){
	printf("process_request: Error has occured\n");
	close(connectionfd);
	free(request);
	Pthread_exit(NULL);
    }
    ...
    while( (n = Rio_readn_w(serverfd, buf, MAXLINE)) > 0 ) {
	response_len += n;
	Rio_writen_w(connectionfd, buf, n);
#if defined(DEBUG)	
	printf("Forwarded %d bytes from end server to client\n", n);
	fflush(stdout);
#endif
    ...
    fprintf(log_file, "%s\n", log_entry);
    fflush(log_file);
    pthread_mutex_unlock(&mutex_lock); //unlocks when finished 
